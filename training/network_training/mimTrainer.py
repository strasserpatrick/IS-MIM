...
from nnunet.utilities.nd_softmax import identity

from nnunet.utilities.to_torch import maybe_to_torch, to_cuda

from nnunet.network_architecture.mask_image_modeling import *
...
class MimTrainer(NetworkTrainer):
    def __init__(
        self,
        plans_file,
        fold,
        ...
        if mask_size is not None:
            self.mask_size = (int(mask_size), int(mask_size), int(mask_size))
        ...
        self.num_input_channels = self.num_classes = self.net_pool_per_axis = (
            self.patch_size
        ) = self.batch_size = self.threeD = self.base_num_features = (
            self.intensity_properties
        ) = self.normalization_schemes = self.net_num_pool_op_kernel_sizes = (
        ...
        self.classes = self.do_dummy_2D_aug = self.use_mask_for_norm = (
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        self.print_to_log_file("synthesize:     | ", self.synthesize)
        self.print_to_log_file("mask_type:      | ", self.mask_type)
        self.print_to_log_file("mask_size:      | ", self.mask_size)
        self.print_to_log_file("mask_ratio:     | ", self.mask_ratio)
        self.print_to_log_file("loss type:      | ", self.loss_type)
    ...
    def preprocess_patient(self, input_files):
        ...
        assert preprocessor_class is not None, (
            "Could not find preprocessor %s in nnunet.preprocessing" % preprocessor_name
        )
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(MimTrainer, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
    ...
    def gene_mask(self, x, phase, mask_seq):
        if phase is None:
            return x, None
        elif phase == "block_wise":
            x, mask_seq = random_block_masking_generator(
                x, self.mask_ratio, self.mask_size, mask_seq
            )
            return x, mask_seq
        elif phase == "constant_channel":
            x, mask_seq = constant_channel(
                x, self.mask_ratio, self.mask_size[0], mask_seq
            )
            return x, mask_seq
        elif phase == "channel_wise":
            x, mask_seq = channel_wise(x, self.mask_ratio, mask_seq)
            return x, mask_seq
        else:
            raise ValueError(
                f'Get phase {phase}, needed to be "block_wise" "constant_channel" "channel_wise"'
            )
    ...
    def run_training(self):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.epoch % 100 == 0 and self.epoch != 0:
                self.save_model_naive(
                    filename=join(self.output_folder, f"{int(self.epoch)}.pth")
                )

        self.epoch -= 1  # if we don't do this we can get a problem with loading model_final_checkpoint.

        if self.save_final_checkpoint:
            self.save_model_naive(join(self.output_folder, "model_final.pth"))
    ...
    def run_iteration(
        ...
        if self.mask_type is not None:
            data, mask_seq = self.gene_mask(data, self.mask_type, None)
        else:
            data = data
            mask_seq = None
        ...
        if self.fp16:
            with autocast():
                ...
                if self.loss_type == "predict":
                    target, _ = self.gene_mask(
                        target, self.mask_type, [1 - x for x in mask_seq]
                    )
                    output, _ = self.gene_mask(
                        output, self.mask_type, [1 - x for x in mask_seq]
                    )
                else:
                    pass
    ...
    def validate(self, validation_folder_name: str = "validation_raw", overwrite=True):
        ...
        for k in self.dataset_tr.keys():
            ...
            if overwrite or (not isfile(join(output_folder, fname + ".nii.gz"))):
                ...
                sitk.WriteImage(
                    t1_ce, os.path.join(output_folder, fname, "t1_ce.nii.gz")
                )
                sitk.WriteImage(t2, os.path.join(output_folder, fname, "t2.nii.gz"))
                sitk.WriteImage(
                    flair, os.path.join(output_folder, fname, "flair.nii.gz")
                )
                ...
                sitk.WriteImage(
                    t1,
                    os.path.join(
                        self.output_folder_base, "gt_niftis", fname, "t1.nii.gz"
                    ),
                )
                sitk.WriteImage(
                    t1_ce,
                    os.path.join(
                        self.output_folder_base, "gt_niftis", fname, "t1_ce.nii.gz"
                    ),
                )
                sitk.WriteImage(
                    t2,
                    os.path.join(
                        self.output_folder_base, "gt_niftis", fname, "t2.nii.gz"
                    ),
                )
                sitk.WriteImage(
                    flair,
                    os.path.join(
                        self.output_folder_base, "gt_niftis", fname, "flair.nii.gz"
                    ),
                )
                ...
                maybe_mkdir_p(
                    os.path.join(self.output_folder_base, "aug_nifitis", fname)
                )
                sitk.WriteImage(
                    t1,
                    os.path.join(
                        self.output_folder_base, "aug_nifitis", fname, "t1.nii.gz"
                    ),
                )
                sitk.WriteImage(
                    t1_ce,
                    os.path.join(
                        self.output_folder_base, "aug_nifitis", fname, "t1_ce.nii.gz"
                    ),
                )
                sitk.WriteImage(
                    t2,
                    os.path.join(
                        self.output_folder_base, "aug_nifitis", fname, "t2.nii.gz"
                    ),
                )
                sitk.WriteImage(
                    flair,
                    os.path.join(
                        self.output_folder_base, "aug_nifitis", fname, "flair.nii.gz"
                    ),
                )
